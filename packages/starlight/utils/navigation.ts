...
// TODO(HiDeoo) rename/comment
// TODO(HiDeoo) think about using a different data structure (LRU cache, WeakMap, etc.)
const autogeneratedTreesPerLocalDir = new Map<string, Dir>();
...
function groupFromAutogenerateConfig(
	...
	// TODO(HiDeoo) Remove this when done benchmarking
	let tree: Dir | undefined;
	if (import.meta.env.NEW_TREEIFY) {
		tree = autogeneratedTreesPerLocalDir.get(localeDir);
		if (!tree) {
			tree = treeify(dirDocs, localeDir);
			autogeneratedTreesPerLocalDir.set(localeDir, tree);
		}
	} else {
		tree = treeify(dirDocs, localeDir);
	}
